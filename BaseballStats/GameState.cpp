...
void GameState::updateBaserunners(const PlayRecord* play_)
{
	...
	if (line_parsed_period.size() > 1) {
		...
		for (auto&& m_str : line_parsed_semi) {
			...
			std::vector<long long> out_positions; //Store play, eg 92 (RF to Catcher) as single long, rundowns are too long for string
			for (auto&& details : line_parsed_details) {
				...
				if (found_error_local == false) {
					...
					if (position_start > 0 && position_start < 10) {
						const long long positions_out_local = string2longlong(details);
